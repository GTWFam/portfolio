import { trackEvent } from "./GAtracking";
...
function drawShape(aShape, offset) {
  aShape.forEach((row, y) => {
    row.forEach((value, x) => {
      if (
        (aShape.length > 4 || value !== 0) &&
        context !== null &&
        canvas !== null
      ) {
        context.fillStyle = colors[value];
        context.fillRect(x + offset.x + 0.1, y + offset.y, 0.9, 0.9);
...
function submitScore() {
  trackEvent("New Tetris Score Submitted");
...
function playerReset() {
  ...
  player.pos.y = 0;
  player.pos.x =
    ((arena[0].length / 2) | 0) - ((player.currentShape[0].length / 2) | 0);
  if (shapeCollide(arena, player)) {
